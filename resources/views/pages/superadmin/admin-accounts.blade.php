<x-layouts.app :title="__('Admin Accounts')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        <x-table>
                ...
                <x-table-row class="">
                    ...
                    <td class="border-b border-gray-100 px-4 py-2 text-center">
                        <flux:modal.trigger name="edit-admin">
                            <a href="javascript:;" id="edit-admin" 
                                data-id="{{ $ad->encrypted_id }}"
                                data-name="{{ $ad->name }}"
                                data-username="{{ $ad->email }}"
                                >
            ...
            @if ($admins->isEmpty())
                <x-table-row>
                    <td colspan="4" class="border-b border-gray-100 px-4 py-2 text-center text-gray-500">No admin accounts
                        found.</td>
                </x-table-row>
            @endif
        </x-table>
    ...
    <x-modal name="delete-admin" class="w-auto md:m-auto mx-4">
        ...
        <div class="flex">
            <flux:spacer />
            <flux:button type="button" variant="outline" class="me-3"
                x-on:click="$flux.modal('delete-admin').close()">Cancel</flux:button>
